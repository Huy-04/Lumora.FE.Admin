...
On this Windows checkout, use `scripts/harness.cmd` from PowerShell or
`cmd.exe`. The POSIX `scripts/harness` wrapper is kept for shell-compatible
environments.
...
Requires: the prebuilt Rust CLI at `scripts/bin/harness-cli` on POSIX shells or
